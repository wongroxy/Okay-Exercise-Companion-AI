...
import { BACKGROUNDTILE_URL, GOOGLE_CLIENT_ID } from './config';
...
function AppContent() {
  ...
  const handleGradeQuiz = useCallback(async () => {
    ...
    try {
      ...
      await saveQuizSession(result, dataUrls);
  ...
  const handleGradeEssay = useCallback(async () => {
    ...
    try {
      ...
      const dataUrls = await Promise.all(imageFiles.map(fileToDataUrl));
      await saveEssay(result, dataUrls);
            ...
            <div className="mb-6">
              ...
              <div className="relative flex w-full max-w-md mx-auto p-1 bg-gray-200 dark:bg-gray-700 rounded-lg shadow-inner">
                <button
                  ...
                  className={`w-1/3 rounded-md py-2 text-sm font-medium transition-colors focus:outline-none ${selectedModel === 'gemini-2.5-flash' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                    }`}
                >
                ...
                <button
                  ...
                  className={`w-1/3 rounded-md py-2 text-sm font-medium transition-colors focus:outline-none ${selectedModel === 'gemini-2.5-pro' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                    }`}
                >
                ...
                <button
                  ...
                  className={`w-1/3 rounded-md py-2 text-sm font-medium transition-colors focus:outline-none ${selectedModel === 'gemini-3-pro-preview' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                    }`}
                >
