...
export const DatabaseScreen: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        const quizzes = await getQuizSessions();
        const essays = await getSavedEssays();
    ...
    const handleUpdateQuestion = useCallback(async (
        ...
        await updateReviewQuestion(sessionId, questionId, updates);
            ...
            {hasData && (
                <div className="mb-6 flex justify-center p-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                    <button
                        ...
                        className={`w-1/2 rounded-md py-2 text-sm font-medium transition-colors focus:outline-none ${view === 'quizzes' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`w-1/2 rounded-md py-2 text-sm font-medium transition-colors focus:outline-none ${view === 'essays' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                            }`}
                    >
                ...
                <div className="mb-6 flex justify-center p-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                    {(['notSolved', 'solved', 'all'] as FilterType[]).map(f => (
                        <button
                            ...
                            className={`w-1/3 rounded-md py-2 text-sm font-medium capitalize transition-colors focus:outline-none ${quizFilter === f ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
                                }`}
                        >
                            {t(`database.filter.${f}`)}
                        </button>
